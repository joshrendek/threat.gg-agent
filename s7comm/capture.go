...
// Session progression stages, in forward-only order -- see session.advance.
...
// stageOrder ranks each stage so advance() can refuse to move reachedStage
// backwards -- e.g. a data read served after a CPU STOP must not erase the
// fact that the attacker already reached "control", the deepest and most
// valuable stage.
...
const (
	...
	// never stored -- see session.record and session.toProto.
	maxOperations = 256
...
// operation is one handled or unhandled S7 request this honeypot answered
// (or couldn't) during a session. handled=false + raw is deliberately an
// unhandled-request queue: any function code or SZL index the emulator
// could not answer, captured verbatim -- see S7CommOperation's own doc
// comment in the proto for why this is the highest-value output of the
// whole feature.
...
// single record sent at disconnect (see persistSession). It is only ever
// touched by the one goroutine driving handleConnection, so no locking is
// needed here -- unlike state.go's attackerState, which is genuinely shared
// across connections from the same attacker IP.
type session struct {
	srcTSAP           string
	dstTSAP           string
...
func newSession() *session {
	return &session{reachedStage: stageConnected, startedAt: time.Now()}
}
...
func (s *session) advance(stage string) {
	if stageOrder[stage] > stageOrder[s.reachedStage] {
...
func (s *session) record(op operation) {
	...
	if len(op.raw) > maxRawBytes {
		op.raw = op.raw[:maxRawBytes]
	}
	if len(op.raw) > 0 {
...
func (s *session) toProto(remoteAddr, guid string) *proto.S7CommSessionRequest {
	ops := make([]*proto.S7CommOperation, len(s.operations))
	for i, op := range s.operations {
		detail := op.detail
		if s.droppedOperations > 0 && i == len(s.operations)-1 {
			...
			detail = fmt.Sprintf("%s [+%d further operations dropped past the %d-operation cap]", detail, s.droppedOperations, maxOperations)
		}
	...
	return &proto.S7CommSessionRequest{
		...
		ReachedStage:      s.reachedStage,
		Operations:        ops,
		DurationMs:        uint32(time.Since(s.startedAt).Milliseconds()),
	}
}
...
func persistSession(logger zerolog.Logger, remoteAddr string, sess *session) {
	if sess.reachedStage == stageConnected {
