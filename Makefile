...
build:
	...
	@echo "*****"
	mkdir -p dist
	GOOS=linux CGO_ENABLED=0 go build -a -installsuffix cgo -ldflags '-s' -ldflags="-extldflags=-static" \
	...
	GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -a -installsuffix cgo -ldflags '-s' -ldflags="-extldflags=-static" \
	  -ldflags "-X main.Version=`date -u +%Y%m%d`.${BUILD_NUMBER}" -o dist/${BINARY}_arm64
	GOOS=linux GOARCH=arm CGO_ENABLED=0 go build -a -installsuffix cgo -ldflags '-s' -ldflags="-extldflags=-static" \
